...
test('sales review SOP toolbar removes the rule count and uses the factory custom sort options', () => {
  ...
  assert.ok(runtime.includes('data-sales-review-rule-search'));
  assert.ok(runtime.includes('data-sales-review-rule-sort-trigger'));
  assert.ok(runtime.includes('data-sales-review-rule-sort-panel'));
  ...
  assert.ok(runtime.includes("state.sopReviewRuleSort = option.dataset.salesReviewRuleSort || 'rate-desc'"));
  assert.ok(runtime.includes('renderSalesSopReview(role)'));
  assert.ok(runtime.includes('state.sopReviewRuleQuery = nextQuery'));
