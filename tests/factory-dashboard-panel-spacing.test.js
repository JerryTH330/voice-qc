...
test('factory dashboard primary panels keep 20px inset except the latest Figma issue overview', () => {
  ...
  assert.match(layoutCss, /\.global-filter-bar\.session-filter-card,[\s\S]*?\.hero-panel,[\s\S]*?\.sop-overview-track,[\s\S]*?\.sop-rank-track,[\s\S]*?\.store-trend-card,[\s\S]*?#panel-sop-improvement \.contribution-track\s*{[\s\S]*?padding:\s*20px;/);
  ...
  assert.match(layoutCss, /#panel-sop-improvement \.contribution-track \.track-header,[\s\S]*?#panel-sop-improvement \.contribution-track \.track-body\s*{[\s\S]*?padding:\s*0;/);
});
